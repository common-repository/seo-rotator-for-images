 ...
 */
if ( !class_exists( 'super_Tracking' ) ) {
	class super_Tracking {

		/**
		 * Class constructor
		 */
		function __construct() {
			add_action( 'super_tracking', array( $this, 'tracking' ) );
		}
		...
		function tracking() {
			...
			$hash = get_option( 'super_Tracking_Hash' );

			if ( !isset( $hash ) || !$hash || empty( $hash ) ) {
				$hash = md5( site_url() );
				update_option( 'super_Tracking_Hash', $hash );
			}

			$data = get_transient( 'super_tracking_cache' );
			if ( !$data ) {
				...
				$data = array(
					...
					'options'  => apply_filters( 'super_tracking_filters', array() ),
					'theme'    => $theme,
					'plugins'  => $plugins,
				);
				...
				wp_remote_post( 'https://tracking.superseo.com/', $args );

				// Store for a week, then push data again.
				set_transient( 'super_tracking_cache', true, 7 * 60 * 60 * 24 );
			}
		}
	}

	$super_tracking = new super_Tracking;
}
...
add_filter( 'super_tracking_filters', 'wpseo_tracking_additions' );