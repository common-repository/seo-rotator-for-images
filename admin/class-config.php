...
class WPSEO_Admin_Pages {
	...
	/**
	 * Resets the site to the default Super SEO settings and runs a title test to check whether force rewrite needs to be on.
	 */
	 ...
	 */
	function admin_header( $form = true, $option = 'super_wpseo_options', $optionshort = 'wpseo', $contains_files = false ) {
		?>
		<div class="wrap">
		<?php
		/**
	...
	function export_settings( $include_taxonomy ) {
		$content = "; " . __( "This is a settings export file for the Super SEO plugin by superseo.comseo.com", 'seo-rotator-for-images' ) . " - http://superseo.com/wordpress/seo/ \r\n";
	...
	function config_page_styles() {
		...
		if ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && in_array( $_GET['page'], $this->adminpages ) ) {
			...
			wp_enqueue_style( 'super-admin-css', plugins_url( 'css/spr_plugin_tools.css', dirname( __FILE__ ) ), array(), WPSEO_VERSION );
	...
	function postbox( $id, $title, $content ) {
		?>
		<div id="<?php echo esc_attr( $id ); ?>" class="superbox">
