...
class WPSEO_Metabox {
	...
	function __construct() {
		if ( ! class_exists( 'super_TextStatistics' ) && apply_filters( 'wpseo_use_page_analysis', true ) === true )
	...
	public function publish_box() {
		echo '<div class="misc-pub-section misc-super misc-pub-section-last">';

		if ( (int) wpseo_get_value( 'meta-robots-noindex' ) === 1 ) {
			$score_label = 'noindex';
			$title       = __( 'Post is set to noindex.', 'seo-rotator-for-images' );
		}
		else {
			...
			if ( $score ) {
				...
				if ( ! $score || empty( $score ) ) {
					$score_label = 'na';
					$title       = __( 'No focus keyword set.', 'seo-rotator-for-images' );
				}
				else {
					$score_label = wpseo_translate_score( $score );
				}
		...
		echo __( 'SEO: ', 'seo-rotator-for-images' ) . $result . ' <a class="wpseo_tablink scroll" href="#wpseo_linkdex">' . __( 'Check', 'seo-rotator-for-images' ) . '</a>';

		echo '</div>';
	}

	/**
	 * Adds the Super SEO box to the edit boxes in the edit post / page overview.
	 */
	public function add_custom_box() {
		...
		foreach ( get_post_types( array( 'public' => true ) ) as $posttype ) {
			if ( isset( $options['hideeditbox-' . $posttype] ) && $options['hideeditbox-' . $posttype] )
				continue;
			add_meta_box( 'wpseo_meta', __( 'Super SEO by superseo.com', 'seo-rotator-for-images' ), array( $this, 'meta_box' ), $posttype, 'normal', apply_filters( 'wpseo_metabox_prio', 'high' ) );
		}
	}
	...
	public function script() {
		...
		if ( isset( $options['showdate-' . $post->post_type] ) && $options['showdate-' . $post->post_type] ) {
			...
			$this->meta_length_reason = __( ' (because of date display)', 'seo-rotator-for-images' );
		}
	...
	public function get_meta_boxes( $post_type = 'post' ) {
		...
		$mbs['snippetpreview'] = array(
			"name"  => "snippetpreview",
			"type"  => "snippetpreview",
			"title" => __( "Snippet Preview", 'seo-rotator-for-images' ),
		);
		$mbs['focuskw']        = array(
			"name"         => "focuskw",
			"std"          => "",
			"type"         => "text",
			"title"        => __( 'Focus Keyword', 'seo-rotator-for-images' ),
			"autocomplete" => "off",
			"help"         => sprintf( __( "Pick the main keyword or keyphrase that this post / page is about.<br/><br/>Read %sthis post%s for more info.", 'seo-rotator-for-images' ), "<a href='http://superseo.com/focus-keyword/#utm_source=seo-rotator-for-images-metabox&utm_medium=inline-help&utm_campaign=focus-keyword'>", '</a>' ),
		...
		$mbs['title']          = array(
			...
			"std"         => "",
			"type"        => "text",
			"title"       => __( "SEO Title", 'seo-rotator-for-images' ),
			"description" => sprintf( __( "Title display in search engines is limited to 70 chars, %s chars left.", 'seo-rotator-for-images' ), "<span id='super_wpseo_title-length'></span>" ),
			"help"        => __( "The SEO Title defaults to what is generated based on this sites title template for this posttype.", 'seo-rotator-for-images' )
		);
		$mbs['metadesc']       = array(
			"name"        => "metadesc",
			"std"         => "",
			"class"       => "metadesc",
			"type"        => "textarea",
			"title"       => __( "Meta Description", 'seo-rotator-for-images' ),
			"rows"        => 2,
			"richedit"    => false,
			"description" => sprintf( __( "The <code>meta</code> description will be limited to %s chars%s, %s chars left.", 'seo-rotator-for-images' ), $this->meta_length, $this->meta_length_reason, "<span id='super_wpseo_metadesc-length'></span>" ) . " <div id='super_wpseo_metadesc_notice'></div>",
			"help"        => __( "If the meta description is empty, the snippet preview above shows what is generated based on this sites meta description template.", 'seo-rotator-for-images' ),
		);
		if ( isset( $options['usemetakeywords'] ) && $options['usemetakeywords'] ) {
			$mbs['metakeywords'] = array(
				...
				"class"       => "metakeywords",
				"type"        => "text",
				"title"       => __( "Meta Keywords", 'seo-rotator-for-images' ),
				"description" => sprintf( __( "If you type something above it will override your %smeta keywords template%s.", 'seo-rotator-for-images' ), "<a target='_blank' href='" . admin_url( 'admin.php?page=wpseo_titles#' . esc_url( $post_type ) ) . "'>", "</a>" )
			);
		}
	...
	function get_advanced_meta_boxes() {
		...
		$mbs['meta-robots-noindex']  = array(
			...
			"std"     => "-",
			"title"   => __( "Meta Robots Index", 'seo-rotator-for-images' ),
			"type"    => "select",
			"options" => array(
				"0" => sprintf( __( "Default for post type, currently: %s", 'seo-rotator-for-images' ), ( isset( $options['noindex-' . $post_type] ) && $options['noindex-' . $post_type] ) ? 'noindex' : 'index' ),
				"2" => __( "index", 'seo-rotator-for-images' ),
				"1" => __( "noindex", 'seo-rotator-for-images' ),
			),
		);
		$mbs['meta-robots-nofollow'] = array(
			"name"    => "meta-robots-nofollow",
			"std"     => "follow",
			"title"   => __( "Meta Robots Follow", 'seo-rotator-for-images' ),
			"type"    => "radio",
			"options" => array(
				"0" => __( "Follow", 'seo-rotator-for-images' ),
				"1" => __( "Nofollow", 'seo-rotator-for-images' ),
			),
		);
		$mbs['meta-robots-adv']      = array(
			...
			"std"         => "none",
			"type"        => "multiselect",
			"title"       => __( "Meta Robots Advanced", 'seo-rotator-for-images' ),
			"description" => __( "Advanced <code>meta</code> robots settings for this page.", 'seo-rotator-for-images' ),
			"options"     => array(
				"noodp"     => __( "NO ODP", 'seo-rotator-for-images' ),
				"noydir"    => __( "NO YDIR", 'seo-rotator-for-images' ),
				"noarchive" => __( "No Archive", 'seo-rotator-for-images' ),
				"nosnippet" => __( "No Snippet", 'seo-rotator-for-images' ),
			),
		);
		if ( isset( $options['breadcrumbs-enable'] ) && $options['breadcrumbs-enable'] ) {
			$mbs['bctitle'] = array(
				...
				"std"         => "",
				"type"        => "text",
				"title"       => __( "Breadcrumbs title", 'seo-rotator-for-images' ),
				"description" => __( "Title to use for this page in breadcrumb paths", 'seo-rotator-for-images' ),
			);
		}
		if ( isset( $options['enablexmlsitemap'] ) && $options['enablexmlsitemap'] ) {
			$mbs['sitemap-include'] = array(
				...
				"std"         => "-",
				"type"        => "select",
				"title"       => __( "Include in Sitemap", 'seo-rotator-for-images' ),
				"description" => __( "Should this page be in the XML Sitemap at all times, regardless of Robots Meta settings?", 'seo-rotator-for-images' ),
				"options"     => array(
					"-"      => __( "Auto detect", 'seo-rotator-for-images' ),
					"always" => __( "Always include", 'seo-rotator-for-images' ),
					"never"  => __( "Never include", 'seo-rotator-for-images' ),
				),
			);
			$mbs['sitemap-prio']    = array(
				...
				"std"         => "-",
				"type"        => "select",
				"title"       => __( "Sitemap Priority", 'seo-rotator-for-images' ),
				"description" => __( "The priority given to this page in the XML sitemap.", 'seo-rotator-for-images' ),
				"options"     => array(
					"-"   => __( "Automatic prioritization", 'seo-rotator-for-images' ),
					"1"   => __( "1 - Highest priority", 'seo-rotator-for-images' ),
					"0.9" => "0.9",
					"0.8" => "0.8 - " . __( "Default for first tier pages", 'seo-rotator-for-images' ),
					"0.7" => "0.7",
					"0.6" => "0.6 - " . __( "Default for second tier pages and posts", 'seo-rotator-for-images' ),
					"0.5" => "0.5 - " . __( "Medium priority", 'seo-rotator-for-images' ),
					"0.4" => "0.4",
					"0.3" => "0.3",
					"0.2" => "0.2",
					"0.1" => "0.1 - " . __( "Lowest priority", 'seo-rotator-for-images' ),
				),
			);
		}
		$mbs['sitemap-html-include'] = array(
			...
			"std"         => "-",
			"type"        => "select",
			"title"       => __( "Include in HTML Sitemap", 'seo-rotator-for-images' ),
			"description" => __( "Should this page be in the HTML Sitemap at all times, regardless of Robots Meta settings?", 'seo-rotator-for-images' ),
			"options"     => array(
				"-"      => __( "Auto detect", 'seo-rotator-for-images' ),
				"always" => __( "Always include", 'seo-rotator-for-images' ),
				"never"  => __( "Never include", 'seo-rotator-for-images' ),
		...
		$mbs['canonical']            = array(
			...
			"std"         => "",
			"type"        => "text",
			"title"       => __( "Canonical URL", 'seo-rotator-for-images' ),
			"description" => sprintf( __( "The canonical URL that this page should point to, leave empty to default to permalink. %sCross domain canonical%s supported too.", 'seo-rotator-for-images' ), "<a target='_blank' href='http://googlewebmastercentral.blogspot.com/2009/12/handling-legitimate-cross-domain.html'>", "</a>" )
		);
		$mbs['redirect']             = array(
			"name"        => "redirect",
			"std"         => "",
			"type"        => "text",
			"title"       => __( "301 Redirect", 'seo-rotator-for-images' ),
			"description" => __( "The URL that this page should redirect to.", 'seo-rotator-for-images' )
		);
	...
	function meta_box() {
		...
		<ul class="wpseo-metabox-tabs" id="wpseo-metabox-tabs">
			<li class="general"><a class="wpseo_tablink"
														 href="#wpseo_general"><?php _e( "General", 'seo-rotator-for-images' ); ?></a></li>
			<li id="linkdex" class="linkdex"><a class="wpseo_tablink"
																					href="#wpseo_linkdex"><?php _e( "Page Analysis", 'seo-rotator-for-images' ); ?></a>
			</li>
			<?php if ( current_user_can( 'manage_options' ) || ! isset( $options['disableadvanced_meta'] ) || ! $options['disableadvanced_meta'] ): ?>
				<li class="advanced"><a class="wpseo_tablink"
																href="#wpseo_advanced"><?php _e( "Advanced", 'seo-rotator-for-images' ); ?></a></li>
			<?php endif; ?>
		...
		$this->do_tab( 'general', __( 'General', 'seo-rotator-for-images' ), $content );

		$this->do_tab( 'linkdex', __( 'Page Analysis', 'seo-rotator-for-images' ), $this->linkdex_output( $post ) );
		...
		if ( current_user_can( 'manage_options' ) || ! isset( $options['disableadvanced_meta'] ) || ! $options['disableadvanced_meta'] ) {
			...
			$this->do_tab( 'advanced', __( 'Advanced', 'seo-rotator-for-images' ), $content );
		}
	...
	function do_meta_box( $meta_box ) {
		...
		if ( isset( $meta_box['help'] ) && $meta_box['help'] )
			$help = '<img src="' . plugins_url( 'images/question-mark.png', dirname( __FILE__ ) ) . '" class="alignright super_help" id="' . $meta_box['name'] . 'help" alt="' . esc_attr( $meta_box['help'] ) . '" />';

		$content .= '<tr>';
		$content .= '<th scope="row"><label for="super_wpseo_' . $meta_box['name'] . '">' . $meta_box['title'] . ':</label>' . $help . '</th>';
		$content .= '<td>';
		...
		switch ( $meta_box['type'] ) {
			...
			case "text":
				...
				if ( isset( $meta_box['autocomplete'] ) && $meta_box['autocomplete'] == 'off' )
					$ac = 'autocomplete="off" ';
				$content .= '<input type="text" placeholder="' . esc_attr( $placeholder ) . '" id="super_wpseo_' . $meta_box['name'] . '" ' . $ac . 'name="super_wpseo_' . $meta_box['name'] . '" value="' . esc_attr( $meta_box_value ) . '" class="large-text"/><br />';
				break;
			case "textarea":
				$content .= '<textarea class="large-text" rows="3" id="super_wpseo_' . $meta_box['name'] . '" name="super_wpseo_' . $meta_box['name'] . '">' . esc_textarea( $meta_box_value ) . '</textarea>';
				break;
			case "select":
				$content .= '<select name="super_wpseo_' . $meta_box['name'] . '" id="super_wpseo_' . $meta_box['name'] . '" class="super' . $class . '">';
			...
			case "multiselect":
				...
				$content .= '<select multiple="multiple" size="' . count( $meta_box['options'] ) . '" style="height: ' . ( count( $meta_box['options'] ) * 16 ) . 'px;" name="super_wpseo_' . $meta_box['name'] . '[]" id="super_wpseo_' . $meta_box['name'] . '" class="super' . $class . '">';
				foreach ( $meta_box['options'] as $val => $option ) {
					$selected = '';
			...
			case "checkbox":
				...
				$content .= '<input type="checkbox" id="super_wpseo_' . $meta_box['name'] . '" name="super_wpseo_' . $meta_box['name'] . '" ' . $checked . ' class="super' . $class . '"/> ' . $expl . '<br />';
				break;
			case "radio":
				if ( $meta_box_value == '' )
					$meta_box_value = $meta_box['std'];
				foreach ( $meta_box['options'] as $val => $option ) {
					...
					$content .= '<input type="radio" ' . $selected . ' id="super_wpseo_' . $meta_box['name'] . '_' . esc_attr( $val ) . '" name="super_wpseo_' . $meta_box['name'] . '" value="' . esc_attr( $val ) . '"/> <label for="super_wpseo_' . $meta_box['name'] . '_' . $val . '">' . $option . '</label> ';
				}
				break;
			case "upload":
				if ( $meta_box_value == '' )
				...
				$content .= '<input id="super_wpseo_'.$meta_box['name'].'" type="text" size="36" name="super_wpseo_'.$meta_box['name'].'" value="' . $meta_box_value . '" />';
				$content .= '<input id="super_wpseo_'.$meta_box['name'].'_button" class="wpseo_image_upload_button button" type="button" value="Upload Image" />';
				$content .= '</label>';
				break;
			case "divtext":
...
//			$content .= '<span href="#" style="font-size: 13px; color: #282; line-height: 15px;" class="breadcrumb">' . super_breadcrumb('','',false) . '</span>';
//		} else {
...
//		$content .= '<pre>' . print_r( $gplus, 1 ) . '</pre>';
	...
	function get_gplus_data( $user_id ) {
		...
		$args = array(
			'headers' => array(
				'Referer' => 'http://superseo.com/wp-admin/',
			),
		);
	...
	function save_postdata( $post_id ) {
		...
		foreach ( $metaboxes as $meta_box ) {
			...
			if ( 'checkbox' == $meta_box['type'] ) {
				if ( isset( $_POST['super_wpseo_' . $meta_box['name']] ) )
					$data = 'on';
				else
					$data = 'off';
			}
				...
				if ( isset( $_POST['super_wpseo_' . $meta_box['name']] ) ) {
					if ( is_array( $_POST['super_wpseo_' . $meta_box['name']] ) )
						$data = implode( ",", $_POST['super_wpseo_' . $meta_box['name']] );
					else
						$data = $_POST['super_wpseo_' . $meta_box['name']];
				}
				else {
					continue;
				}
	...
	public function enqueue() {
		...
		if ( $pagenow == 'edit.php' ) {
			...
			wp_localize_script( 'wp-seo-metabox', 'wpseoMetaboxL10n', array(
				'keyword_header'        => __( 'Your focus keyword was found in:', 'seo-rotator-for-images' ),
				'article_header_text'   => __( 'Article Heading: ', 'seo-rotator-for-images' ),
				'page_title_text'       => __( 'Page title: ', 'seo-rotator-for-images' ),
				'page_url_text'         => __( 'Page URL: ', 'seo-rotator-for-images' ),
				'content_text'          => __( 'Content: ', 'seo-rotator-for-images' ),
				'meta_description_text' => __( 'Meta description: ', 'seo-rotator-for-images' ),
				'choose_image'          => __( 'Use Image', 'seo-rotator-for-images' )
			) );
		}
	}
	...
	function posts_filter_dropdown() {
		...
		echo '<option value="">' . __( "All SEO Scores", 'seo-rotator-for-images' ) . '</option>';
		foreach ( array(
								'na'      => __( 'SEO: No Focus Keyword', 'seo-rotator-for-images' ),
								'bad'     => __( 'SEO: Bad', 'seo-rotator-for-images' ),
								'poor'    => __( 'SEO: Poor', 'seo-rotator-for-images' ),
								'ok'      => __( 'SEO: OK', 'seo-rotator-for-images' ),
								'good'    => __( 'SEO: Good', 'seo-rotator-for-images' ),
								'noindex' => __( 'SEO: Post Noindexed', 'seo-rotator-for-images' )
							) as $val => $text ) {
			$sel = '';
	...
	function column_heading( $columns ) {
		return array_merge( $columns, array( 'wpseo-score' => __( 'SEO', 'seo-rotator-for-images' ), 'wpseo-title' => __( 'SEO Title', 'seo-rotator-for-images' ), 'wpseo-metadesc' => __( 'Meta Desc.', 'seo-rotator-for-images' ), 'wpseo-focuskw' => __( 'Focus KW', 'seo-rotator-for-images' ) ) );
	}
	...
	function column_content( $column_name, $post_id ) {
		if ( $column_name == 'wpseo-score' ) {
			if ( (int) wpseo_get_value( 'meta-robots-noindex', $post_id ) === 1 ) {
				...
				if ( ! $score || empty( $score ) ) {
					$score_label = 'na';
					$title       = __( 'Focus keyword not set.', 'seo-rotator-for-images' );
				}
				else {
					$score_label = wpseo_translate_score( $score );
					$title       = wpseo_translate_score( $score, $css = false );
				...
				$vars = array_merge(
					...
					array(
						'meta_query' => array(
							'relation' => 'AND',
							array(
								'key'     => '_super_wpseo_meta-robots-noindex',
								'value'   => 1,
								'compare' => '!='
							),
							array(
								'key'     => '_super_wpseo_linkdex',
	...
	function linkdex_output( $post ) {
		...
		$output = '<div class="wpseo_msg"><p>' . __( 'To update this page analysis, save as draft or update and check this tab again', 'seo-rotator-for-images' ) . '.</p></div>' . $output;
	...
	function calculate_results( $post ) {
		...
		if ( ! class_exists( 'DOMDocument' ) ) {
			$result = new WP_Error( 'no-domdocument', sprintf( __( "Your hosting environment does not support PHP's %sDocument Object Model%s.", 'seo-rotator-for-images' ), '<a href="http://php.net/manual/en/book.dom.php">', '</a>' ) . ' ' . __( "To enjoy all the benefits of the page analysis feature, you'll need to (get your host to) install it.", 'seo-rotator-for-images' ) );
			return $result;
		}

		if ( ! wpseo_get_value( 'focuskw', $post->ID ) ) {
			$result = new WP_Error( 'no-focuskw', sprintf( __( 'No focus keyword was set for this %s. If you do not set a focus keyword, no score can be calculated.', 'seo-rotator-for-images' ), $post->post_type ) );
		...
		elseif ( apply_filters( 'wpseo_use_page_analysis', true ) !== true ) {
			$result = new WP_Error( 'page-analysis-disabled', sprintf( __( 'Page Analysis has been disabled.', 'seo-rotator-for-images' ), $post->post_type ) );
		...
		global $statistics;
		$statistics = new super_TextStatistics;
	...
	function check_double_focus_keyword( $job, &$results ) {
		...
		if ( count( $posts ) == 0 )
			$this->save_score_result( $results, 9, __( "You've never used this focus keyword before, very good.", 'seo-rotator-for-images' ), 'keyword_overused' );
		else if ( count( $posts ) == 1 )
			$this->save_score_result( $results, 6, sprintf( __( 'You\'ve used this focus keyword %1$sonce before%2$s, be sure to make very clear which URL on your site is the most important for this keyword.', 'seo-rotator-for-images' ), '<a href="' . admin_url( 'post.php?post=' . $posts[0] . '&action=edit' ) . '">', '</a>' ), 'keyword_overused' );
		else
			$this->save_score_result( $results, 1, sprintf( __( 'You\'ve used this focus keyword %3$s%4$d times before%2$s, it\'s probably a good idea to read %1$sthis post on cornerstone content%2$s and improve your keyword strategy.', 'seo-rotator-for-images' ), '<a href="http://superseo.com/cornerstone-content-rank/">', '</a>', '<a href="' . admin_url( 'edit.php?seo_kw_filter=' . urlencode( $job['keyword'] ) ) . '">', count( $posts ) ), 'keyword_overused' );
	}
	...
	function score_keyword( $keyword, &$results ) {
		...
		$keywordStopWord = __( "The keyword for this page contains one or more %sstop words%s, consider removing them. Found '%s'.", 'seo-rotator-for-images' );
	...
	function score_url( $job, &$results ) {
		...
		$urlGood      = __( "The keyword / phrase appears in the URL for this page.", 'seo-rotator-for-images' );
		$urlMedium    = __( "The keyword / phrase does not appear in the URL for this page. If you decide to rename the URL be sure to check the old URL 301 redirects to the new one!", 'seo-rotator-for-images' );
		$urlStopWords = __( "The slug for this page contains one or more <a href=\"http://en.wikipedia.org/wiki/Stop_words\">stop words</a>, consider removing them.", 'seo-rotator-for-images' );
		$longSlug     = __( "The slug for this page is a bit long, consider shortening it.", 'seo-rotator-for-images' );

		$needle    = $this->strip_separators_and_fold( $job["keyword"] );
		$hasprack1 = $this->strip_separators_and_fold( $job["pageUrl"], true );
		$hasprack2 = $this->strip_separators_and_fold( $job["pageUrl"], false );

		if ( stripos( $hasprack1, $needle ) || stripos( $hasprack2, $needle ) )
	...
	function score_title( $job, &$results ) {
		...
		$scoreTitleMissing          = __( "Please create a page title.", 'seo-rotator-for-images' );
		$scoreTitleCorrectLength    = __( "The page title is more than 40 characters and less than the recommended 70 character limit.", 'seo-rotator-for-images' );
		$scoreTitleTooShort         = __( "The page title contains %d characters, which is less than the recommended minimum of 40 characters. Use the space to add keyword variations or create compelling call-to-action copy.", 'seo-rotator-for-images' );
		$scoreTitleTooLong          = __( "The page title contains %d characters, which is more than the viewable limit of 70 characters; some words will not be visible to users in your listing.", 'seo-rotator-for-images' );
		$scoreTitleKeywordMissing   = __( "The keyword / phrase %s does not appear in the page title.", 'seo-rotator-for-images' );
		$scoreTitleKeywordBeginning = __( "The page title contains keyword / phrase, at the beginning which is considered to improve rankings.", 'seo-rotator-for-images' );
		$scoreTitleKeywordEnd       = __( "The page title contains keyword / phrase, but it does not appear at the beginning; try and move it to the beginning.", 'seo-rotator-for-images' );
	...
	function score_anchor_texts( $job, &$results, $anchor_texts, $count ) {
		$scoreNoLinks               = __( "No outbound links appear in this page, consider adding some as appropriate.", 'seo-rotator-for-images' );
		$scoreKeywordInOutboundLink = __( "You're linking to another page with the keyword you want this page to rank for, consider changing that if you truly want this page to rank.", 'seo-rotator-for-images' );
		$scoreLinksDofollow         = __( "This page has %s outbound link(s).", 'seo-rotator-for-images' );
		$scoreLinksNofollow         = __( "This page has %s outbound link(s), all nofollowed.", 'seo-rotator-for-images' );
		$scoreLinks                 = __( "This page has %s nofollowed link(s) and %s normal outbound link(s).", 'seo-rotator-for-images' );
	...
	function score_images_alt_text( $job, &$results, $imgs ) {
		$scoreImagesNoImages          = __( "No images appear in this page, consider adding some as appropriate.", 'seo-rotator-for-images' );
		$scoreImagesNoAlt             = __( "The images on this page are missing alt tags.", 'seo-rotator-for-images' );
		$scoreImagesAltKeywordIn      = __( "The images on this page contain alt tags with the target keyword / phrase.", 'seo-rotator-for-images' );
		$scoreImagesAltKeywordMissing = __( "The images on this page do not have alt tags containing your keyword / phrase.", 'seo-rotator-for-images' );
		...
		if ( $imgs['count'] == 0 ) {
			...
			foreach ( $imgs['alts'] as $alt ) {
				$hasprack1 = $this->strip_separators_and_fold( $alt, true );
				$hasprack2 = $this->strip_separators_and_fold( $alt, false );
				if ( strrpos( $hasprack1, $job["keyword_folded"] ) !== false )
					$found = true;
				else if ( strrpos( $hasprack2, $job["keyword_folded"] ) !== false )
					$found = true;
			}
			if ( $found )
	...
	function score_headings( $job, &$results, $headings ) {
		$scoreHeadingsNone           = __( "No subheading tags (like an H2) appear in the copy.", 'seo-rotator-for-images' );
		$scoreHeadingsKeywordIn      = __( "Keyword / keyphrase appears in %s (out of %s) subheadings in the copy. While not a major ranking factor, this is beneficial.", 'seo-rotator-for-images' );
		$scoreHeadingsKeywordMissing = __( "You have not used your keyword / keyphrase in any subheading (such as an H2) in your copy.", 'seo-rotator-for-images' );
		...
		if ( $headingCount == 0 )
			...
			foreach ( $headings as $heading ) {
				$hasprack1 = $this->strip_separators_and_fold( $heading, true );
				$hasprack2 = $this->strip_separators_and_fold( $heading, false );
	...
	function score_description( $job, &$results, $description, $maxlength = 155 ) {
		...
		$scoreDescriptionCorrectLength  = __( "In the specified meta description, consider: How does it compare to the competition? Could it be made more appealing?", 'seo-rotator-for-images' );
		$scoreDescriptionTooShort       = __( "The meta description is under 120 characters, however up to %s characters are available. %s", 'seo-rotator-for-images' );
		$scoreDescriptionTooLong        = __( "The specified meta description is over %s characters, reducing it will ensure the entire description is visible. %s", 'seo-rotator-for-images' );
		$scoreDescriptionMissing        = __( "No meta description has been specified, search engines will display copy from the page instead.", 'seo-rotator-for-images' );
		$scoreDescriptionKeywordIn      = __( "The meta description contains the primary keyword / phrase.", 'seo-rotator-for-images' );
		$scoreDescriptionKeywordMissing = __( "A meta description has been specified, but it does not contain the target keyword / phrase.", 'seo-rotator-for-images' );

		$metaShorter = '';
		if ( $maxlength != 155 )
			$metaShorter = __( "The available space is shorter than the usual 155 characters because Google will also include the publication date in the snippet.", 'seo-rotator-for-images' );
		...
		if ( $description == "" ) {
			...
			// TODO MA Keyword/Title matching is exact match with separators removed, but should extend to distributed match
			$hasprack1 = $this->strip_separators_and_fold( $description, true );
			$hasprack2 = $this->strip_separators_and_fold( $description, false );
			if ( strrpos( $hasprack1, $job["keyword_folded"] ) === false && strrpos( $hasprack2, $job["keyword_folded"] ) === false )
	...
	function score_body( $job, &$results, $body, $firstp ) {
		...
		$scoreBodyGoodLength = __( "There are %d words contained in the body copy, this is more than the %d word recommended minimum.", 'seo-rotator-for-images' );
		$scoreBodyPoorLength = __( "There are %d words contained in the body copy, this is below the %d word recommended minimum. Add more useful content on this topic for readers.", 'seo-rotator-for-images' );
		$scoreBodyOKLength   = __( "There are %d words contained in the body copy, this is slightly below the %d word recommended minimum, add a bit more copy.", 'seo-rotator-for-images' );
		$scoreBodyBadLength  = __( "There are %d words contained in the body copy. This is far too low and should be increased.", 'seo-rotator-for-images' );

		$scoreKeywordDensityLow  = __( "The keyword density is %s%%, which is a bit low, the keyword was found %s times.", 'seo-rotator-for-images' );
		$scoreKeywordDensityHigh = __( "The keyword density is %s%%, which is over the advised 4.5%% maximum, the keyword was found %s times.", 'seo-rotator-for-images' );
		$scoreKeywordDensityGood = __( "The keyword density is %s%%, which is great, the keyword was found %s times.", 'seo-rotator-for-images' );

		$scoreFirstParagraphLow  = __( "The keyword doesn't appear in the first paragraph of the copy, make sure the topic is clear immediately.", 'seo-rotator-for-images' );
		$scoreFirstParagraphHigh = __( "The keyword appears in the first paragraph of the copy.", 'seo-rotator-for-images' );

		$fleschurl   = '<a href="http://en.wikipedia.org/wiki/Flesch-Kincaid_readability_test#Flesch_Reading_Ease">' . __( 'Flesch Reading Ease', 'seo-rotator-for-images' ) . '</a>';
		$scoreFlesch = __( "The copy scores %s in the %s test, which is considered %s to read. %s", 'seo-rotator-for-images' );
		...
		if ( $keywordWordCount > 10 ) {
			$this->save_score_result( $results, 0, __( 'Your keyphrase is over 10 words, a keyphrase should be shorter and there can be only one keyphrase.', 'seo-rotator-for-images' ), 'focus_keyword_length' );
		}
		else {
		...
		if ( substr( $lang, 0, 2 ) == 'en' && $wordCount > 100 ) {
			...
			$score = 1;
			if ( $flesch >= 90 ) {
				$level = __( 'very easy', 'seo-rotator-for-images' );
				$score = 9;
			}
			else if ( $flesch >= 80 ) {
				$level = __( 'easy', 'seo-rotator-for-images' );
				$score = 9;
			}
			else if ( $flesch >= 70 ) {
				$level = __( 'fairly easy', 'seo-rotator-for-images' );
				$score = 8;
			}
			else if ( $flesch >= 60 ) {
				$level = __( 'OK', 'seo-rotator-for-images' );
				$score = 7;
			}
			else if ( $flesch >= 50 ) {
				$level = __( 'fairly difficult', 'seo-rotator-for-images' );
				$note  = __( 'Try to make shorter sentences to improve readability.', 'seo-rotator-for-images' );
				$score = 6;
			}
			else if ( $flesch >= 30 ) {
				$level = __( 'difficult', 'seo-rotator-for-images' );
				$note  = __( 'Try to make shorter sentences, using less difficult words to improve readability.', 'seo-rotator-for-images' );
				$score = 5;
			}
			else if ( $flesch >= 0 ) {
				$level = __( 'very difficult', 'seo-rotator-for-images' );
				$note  = __( 'Try to make shorter sentences, using less difficult words to improve readability.', 'seo-rotator-for-images' );
				$score = 4;
			}
