...
$wpseo_admin_pages->admin_header( true, 'super_wpseo_options', 'wpseo' );
	...
	if ( is_string( $path ) && $path !== '' ) {
		...
		if ( !file_exists( $path . '/' . $backup_file ) ) {
			...
			if ( $backupfile ) {
				...
				fclose( $backupfile );
				$msg = __( 'Backed up the original file header.php to <strong><em>' . $backup_file . '</em></strong>, ', 'seo-rotator-for-images' );
				...
				if ( $count > 0 ) {
					...
					if ( $header_file ) {
						...
						fclose( $header_file );
						$msg .= __( 'Removed hardcoded meta description.', 'seo-rotator-for-images' );
					}
				}
...
if ( isset( $options['blocking_files'] ) && is_array( $options['blocking_files'] ) && count( $options['blocking_files'] ) > 0 ) {
	...
	echo '<p id="blocking_files" class="wrong">'
		. '<a href="javascript:wpseo_killBlockingFiles(\'' . wp_create_nonce( 'wpseo-blocking-files' ) . '\')" class="button fixit">' . __( 'Fix it.', 'seo-rotator-for-images' ) . '</a>'
		. __( 'The following file(s) is/are blocking your XML sitemaps from working properly:', 'seo-rotator-for-images' ) . '<br />';
	foreach ( $options['blocking_files'] as $file ) {
		echo esc_html( $file ) . '<br/>';
	}
	echo __( 'Either delete them (this can be done with the "Fix it" button) or disable WP SEO XML sitemaps.', 'seo-rotator-for-images' );
	echo '</p>';
}
...
if ( isset( $options['theme_check'] ) && isset( $options['theme_check']['description_found'] ) && $options['theme_check']['description_found'] ) {
	echo '<p id="metadesc_found notice" class="wrong settings_error">'
		. '<a href="' . admin_url( 'admin.php?page=wpseo_dashboard&fixmetadesc&nonce=' . wp_create_nonce( 'wpseo-fix-metadesc' ) ) . '" class="button fixit">' . __( 'Fix it.', 'seo-rotator-for-images' ) . '</a>'
		. ' <a href="' . admin_url( 'admin.php?page=wpseo_dashboard&checkmetadesc&nonce=' . wp_create_nonce( 'wpseo-check-metadesc' ) ) . '" class="button checkit">' . __( 'Re-check theme.', 'seo-rotator-for-images' ) . '</a>'
		. __( 'Your theme contains a meta description, which blocks Super SEO from working properly, please delete the following line, or press fix it:', 'seo-rotator-for-images' ) . '<br />';
...
if ( strpos( get_option( 'permalink_structure' ), '%postname%' ) === false && !isset( $options['ignore_permalink'] ) )
	echo '<p id="wrong_permalink" class="wrong">'
		. '<a href="' . admin_url( 'options-permalink.php' ) . '" class="button fixit">' . __( 'Fix it.', 'seo-rotator-for-images' ) . '</a>'
		. '<a href="javascript:wpseo_setIgnore(\'permalink\',\'wrong_permalink\',\'' . wp_create_nonce( 'wpseo-ignore' ) . '\');" class="button fixit">' . __( 'Ignore.', 'seo-rotator-for-images' ) . '</a>'
		. __( 'You do not have your postname in the URL of your posts and pages, it is highly recommended that you do. Consider setting your permalink structure to <strong>/%postname%/</strong>.', 'seo-rotator-for-images' ) . '</p>';

if ( get_option( 'page_comments' ) && !isset( $options['ignore_page_comments'] ) )
	echo '<p id="wrong_page_comments" class="wrong">'
		. '<a href="javascript:setWPOption(\'page_comments\',\'0\',\'wrong_page_comments\',\'' . wp_create_nonce( 'wpseo-setoption' ) . '\');" class="button fixit">' . __( 'Fix it.', 'seo-rotator-for-images' ) . '</a>'
		. '<a href="javascript:wpseo_setIgnore(\'page_comments\',\'wrong_page_comments\',\'' . wp_create_nonce( 'wpseo-ignore' ) . '\');" class="button fixit">' . __( 'Ignore.', 'seo-rotator-for-images' ) . '</a>'
		. __( 'Paging comments is enabled, this is not needed in 999 out of 1000 cases, so the suggestion is to disable it, to do that, simply uncheck the box before "Break comments into pages..."', 'seo-rotator-for-images' ) . '</p>';

echo '<h2>' . __( 'General', 'seo-rotator-for-images' ) . '</h2>';

if ( isset( $options['ignore_tour'] ) && $options['ignore_tour'] ) {
	echo '<label class="select">' . __( 'Introduction Tour:', 'seo-rotator-for-images' ) . '</label><a class="button-secondary" href="' . admin_url( 'admin.php?page=wpseo_dashboard&wpseo_restart_tour' ) . '">' . __( 'Start Tour', 'seo-rotator-for-images' ) . '</a>';
	echo '<p class="desc label">' . __( 'Take this tour to quickly learn about the use of this plugin.', 'seo-rotator-for-images' ) . '</p>';
}

echo '<label class="select">' . __( 'Default Settings:', 'seo-rotator-for-images' ) . '</label><a class="button-secondary" href="' . admin_url( 'admin.php?page=wpseo_dashboard&wpseo_reset_defaults&nonce='. wp_create_nonce( 'wpseo_reset_defaults' ) ) . '">' . __( 'Reset Default Settings', 'seo-rotator-for-images' ) . '</a>';
echo '<p class="desc label">' . __( 'If you want to restore a site to the default Super SEO settings, press this button.', 'seo-rotator-for-images' ) . '</p>';

echo '<h2>' . __( 'Tracking', 'seo-rotator-for-images' ) . '</h2>';
echo $wpseo_admin_pages->checkbox( 'super_tracking', __( 'Allow tracking of this WordPress installs anonymous data.', 'seo-rotator-for-images' ) );
echo '<p class="desc">' . __( "To maintain a plugin as big as Super SEO, we need to know what we're dealing: what kinds of other plugins our users are using, what themes, etc. Please allow us to track that data from your install. It will not track <em>any</em> user details, so your security and privacy are safe with us.", 'seo-rotator-for-images' ) . '</p>';

echo '<h2>' . __( 'Security', 'seo-rotator-for-images' ) . '</h2>';
echo $wpseo_admin_pages->checkbox( 'disableadvanced_meta', __( 'Disable the Advanced part of the Super SEO meta box', 'seo-rotator-for-images' ) );
echo '<p class="desc">' . __( 'Unchecking this box allows authors and editors to redirect posts, noindex them and do other things you might not want if you don\'t trust your authors.', 'seo-rotator-for-images' ) . '</p>';

echo '<h2>' . __( 'Webmaster Tools', 'seo-rotator-for-images' ) . '</h2>';
echo '<p>' . __( 'You can use the boxes below to verify with the different Webmaster Tools, if your site is already verified, you can just forget about these. Enter the verify meta values for:', 'seo-rotator-for-images' ) . '</p>';
echo $wpseo_admin_pages->textinput( 'googleverify', '<a target="_blank" href="https://www.google.com/webmasters/tools/dashboard?hl=en&amp;siteUrl=' . urlencode( get_bloginfo( 'url' ) ) . '%2F">' . __( 'Google Webmaster Tools', 'seo-rotator-for-images' ) . '</a>' );
echo $wpseo_admin_pages->textinput( 'msverify', '<a target="_blank" href="http://www.bing.com/webmaster/?rfp=1#/Dashboard/?url=' . str_replace( 'http://', '', get_bloginfo( 'url' ) ) . '">' . __( 'Bing Webmaster Tools', 'seo-rotator-for-images' ) . '</a>' );
echo $wpseo_admin_pages->textinput( 'alexaverify', '<a target="_blank" href="http://www.alexa.com/pro/subscription">' . __( 'Alexa Verification ID', 'seo-rotator-for-images' ) . '</a>' );
