...
class WPSEO_Sitemaps {
		...
		if ( ! isset( $this->options['disable-author'] ) && ! isset( $this->options['disable_author_sitemap'] ) ) {
			...
			for ( $i = 0; $i < $n; $i ++ ) {
				...
				if ( empty( $count ) || $count == $n ) {
					...
					AND mt1.meta_key = '_super_wpseo_profile_updated' ) ORDER BY mt1.meta_value DESC LIMIT 1
					",
					...
					INNER JOIN $wpdb->usermeta AS mt1 ON ($wpdb->users.ID = mt1.user_id) WHERE 1=1 
					AND ( ($wpdb->usermeta.meta_key = %s AND CAST($wpdb->usermeta.meta_value AS CHAR) != '0')
					AND mt1.meta_key = '_super_wpseo_profile_updated' ) ORDER BY mt1.meta_value ASC LIMIT 1 OFFSET %d
					",
							$wpdb->get_blog_prefix() . 'user_level',
	...
	function build_post_type_map( $post_type ) {
		...
		while ( $total > $offset ) {
			...
			// Optimized query per this thread: http://wordpress.org/support/topic/plugin-seo-rotator-for-images-by-super-performance-suggestion
	...
	function build_user_map() {
		...
		$users = get_users( array(
			...
			'meta_query' => array(
				array(
					'key'     => '_super_wpseo_profile_updated',
		...
		$users = get_users( array(
			...
			'offset'   => $offset,
			'number'   => $steps,
			'meta_key' => '_super_wpseo_profile_updated',
		...
		foreach ( $users as $user ) {
			if ( $author_link = get_author_posts_url( $user->ID ) ) {
				$output .= $this->sitemap_url( array(
					...
					'pri' => 0.8,
					'chf' => 'weekly',
					'mod' => date( 'c', isset( $user->_super_wpseo_profile_updated ) ? $user->_super_wpseo_profile_updated : time() )
				) );
			}
		}
	...
	function output() {
		...
		echo $this->sitemap;
		echo "\n" . '<!-- XML Sitemap generated by superseo.com SEO -->';
	...
	/**
	 * Sorts an array of WP_User by the _super_wpseo_profile_updated meta field
	 *
	 * since 1.6
	 *
	...
	function user_map_sorter( $a, $b ) {
		if ( ! isset( $a->_super_wpseo_profile_updated ) ) {
			$a->_super_wpseo_profile_updated = time();
		}
		if ( ! isset( $b->_super_wpseo_profile_updated ) ) {
			$b->_super_wpseo_profile_updated = time();
		}
		
		if ( $a->_super_wpseo_profile_updated == $b->_super_wpseo_profile_updated ) {
			return 0;
		}
		return ( $a->_super_wpseo_profile_updated > $b->_super_wpseo_profile_updated ) ? 1 : - 1;
	}
