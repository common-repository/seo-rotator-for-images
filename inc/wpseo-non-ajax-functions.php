...
function wpseo_activate() {
	...
	if ( ! function_exists( 'schedule_super_tracking' ) )
		require_once( WPSEO_PATH . 'inc/wpseo-functions.php' );

		
	schedule_super_tracking( null, get_option( 'wpseo' ) );
...
function wpseo_defaults() {
	...
	if ( ! is_array( get_option( 'wpseo_titles' ) ) ) {
		$opt = array(
			...
			'title-author'        => sprintf( __( '%s, Author at %s', 'seo-rotator-for-images' ), '%%name%%', '%%sitename%%' ) . ' %%page%% ',
			'title-archive'       => '%%date%% %%page%% %%sep%% %%sitename%%',
			'title-search'        => sprintf( __( 'You searched for %s', 'seo-rotator-for-images' ), '%%searchphrase%%' ) . ' %%page%% %%sep%% %%sitename%%',
			'title-404'           => __( 'Page Not Found', 'seo-rotator-for-images' ) . ' %%sep%% %%sitename%%',
			'noindex-archive'     => 'on',
		...
		foreach ( get_post_types( array( 'public' => true ), 'objects' ) as $pt ) {
			...
			if ( $pt->has_archive )
				$opt['title-ptarchive-' . $pt->name] = sprintf( __( '%s Archive', 'seo-rotator-for-images' ), '%%pt_plural%%' ) . ' %%page%% %%sep%% %%sitename%%';
		}
		foreach ( get_taxonomies( array( 'public' => true ) ) as $tax ) {
			$opt['title-' . $tax] = sprintf( __( '%s Archives', 'seo-rotator-for-images' ), '%%term_title%%' ) . ' %%page%% %%sep%% %%sitename%%';
		}
	...
	if ( ! is_array( get_option( 'wpseo_rss' ) ) ) {
		$opt = array(
			'rssafter' => sprintf( __( 'The post %s appeared first on %s.', 'seo-rotator-for-images' ), '%%POSTLINK%%', '%%BLOGLINK%%' ),
		);
	...
	// Force WooThemes to use Super SEO data.
...
function wpseo_title_test() {
	...
	$expected_title = 'This is a super Test Title';
	...
	global $wp_version;
	$args = array(
		'user-agent' => "WordPress/${wp_version}; " . get_site_url() . " - super",
	);
 ...
 *
 * @since 2.0.14 Moved from dashboard.php and adjusted - see changelog
 *
 * @return void
 */
...
function wpseo_translate_score( $val, $css = true ) {
	switch ( $val ) {
		case 0:
			$score = __( 'N/A', 'seo-rotator-for-images' );
			$css   = 'na';
			break;
		case 4:
		case 5:
			$score = __( 'Poor', 'seo-rotator-for-images' );
			$css   = 'poor';
			break;
		case 6:
		case 7:
			$score = __( 'OK', 'seo-rotator-for-images' );
			$css   = 'ok';
			break;
		case 8:
		case 9:
		case 10:
			$score = __( 'Good', 'seo-rotator-for-images' );
			$css   = 'good';
			break;
		default:
			$score = __( 'Bad', 'seo-rotator-for-images' );
			$css   = 'bad';
	}
...
function wpseo_admin_bar_menu() {
	...
	$wp_admin_bar->add_menu( array( 'id' => 'wpseo-menu', 'title' => __( 'SEO', 'seo-rotator-for-images' ) . $score, 'href' => $seo_url, ) );
	$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-menu', 'id' => 'wpseo-kwresearch', 'title' => __( 'Keyword Research', 'seo-rotator-for-images' ), '#', ) );
	$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-kwresearch', 'id' => 'wpseo-adwordsexternal', 'title' => __( 'AdWords External', 'seo-rotator-for-images' ), 'href' => 'https://adwords.google.com/select/KeywordToolExternal', 'meta' => array( 'target' => '_blank' ) ) );
	$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-kwresearch', 'id' => 'wpseo-googleinsights', 'title' => __( 'Google Insights', 'seo-rotator-for-images' ), 'href' => 'http://www.google.com/insights/search/#q=' . urlencode( $focuskw ) . '&cmpt=q', 'meta' => array( 'target' => '_blank' ) ) );
	$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-kwresearch', 'id' => 'wpseo-wordtracker', 'title' => __( 'SEO Book', 'seo-rotator-for-images' ), 'href' => 'http://tools.seobook.com/keyword-tools/seobook/?keyword=' . urlencode( $focuskw ), 'meta' => array( 'target' => '_blank' ) ) );

	if ( ! is_admin() ) {
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-menu', 'id' => 'wpseo-analysis', 'title' => __( 'Analyze this page', 'seo-rotator-for-images' ), '#', ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-analysis', 'id' => 'wpseo-inlinks-ose', 'title' => __( 'Check Inlinks (OSE)', 'seo-rotator-for-images' ), 'href' => 'http://www.opensiteexplorer.org/' . str_replace( '/', '%252F', preg_replace( '`^http[s]?://`', '', $url ) ) . '/a!links', 'meta' => array( 'target' => '_blank' ) ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-analysis', 'id' => 'wpseo-kwdensity', 'title' => __( 'Check Keyword Density', 'seo-rotator-for-images' ), 'href' => 'http://tools.davidnaylor.co.uk/keyworddensity/index.php?url=' . $url . '&keyword=' . urlencode( $focuskw ), 'meta' => array( 'target' => '_blank' ) ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-analysis', 'id' => 'wpseo-cache', 'title' => __( 'Check Google Cache', 'seo-rotator-for-images' ), 'href' => 'http://webcache.googleusercontent.com/search?strip=1&q=cache:' . $url, 'meta' => array( 'target' => '_blank' ) ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-analysis', 'id' => 'wpseo-header', 'title' => __( 'Check Headers', 'seo-rotator-for-images' ), 'href' => 'http://quixapp.com/headers/?r=' . urlencode( $url ), 'meta' => array( 'target' => '_blank' ) ) );
	}
	...
	if ( $admin_menu ) {
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-menu', 'id' => 'wpseo-settings', 'title' => __( 'SEO Settings', 'seo-rotator-for-images' ), 'href' => admin_url( 'admin.php?page=wpseo_titles' ), ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-settings', 'id' => 'wpseo-titles', 'title' => __( "Titles & Metas", 'seo-rotator-for-images' ), 'href' => admin_url( 'admin.php?page=wpseo_titles' ), ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-settings', 'id' => 'wpseo-social', 'title' => __( 'Social', 'seo-rotator-for-images' ), 'href' => admin_url( 'admin.php?page=wpseo_social' ), ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-settings', 'id' => 'wpseo-xml', 'title' => __( 'XML Sitemaps', 'seo-rotator-for-images' ), 'href' => admin_url( 'admin.php?page=wpseo_xml' ), ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-settings', 'id' => 'wpseo-permalinks', 'title' => __( 'Permalinks', 'seo-rotator-for-images' ), 'href' => admin_url( 'admin.php?page=wpseo_permalinks' ), ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-settings', 'id' => 'wpseo-internal-links', 'title' => __( 'Internal Links', 'seo-rotator-for-images' ), 'href' => admin_url( 'admin.php?page=wpseo_internal-links' ), ) );
		$wp_admin_bar->add_menu( array( 'parent' => 'wpseo-settings', 'id' => 'wpseo-rss', 'title' => __( 'RSS', 'seo-rotator-for-images' ), 'href' => admin_url( 'admin.php?page=wpseo_rss' ), ) );
	}
}
...
function allow_custom_field_edits( $allcaps, $cap, $args ) {
	...
	if ( in_array( $args[0], array( "edit_post_meta", "add_post_meta" ) ) ) {
		...
		// the meta value starts with _super_wpseo.
		if ( current_user_can( 'edit_post', $args[2] ) && ( ! empty( $args[3] ) && strpos( $args[3], "_super_wpseo_" ) === 0 ) )
			$allcaps[$args[0]] = true;
	}
...
function wpseo_sitemap_handler( $atts ) {
	...
	// create author list
	if ( $display_authors ) {
		$output .= '<h2 id="authors">' . __( 'Authors', 'seo-rotator-for-images' ) . '</h2><ul>';
	...
	// create page list
	if ( $display_pages ) {
		$output .= '<h2 id="pages">' . __( 'Pages', 'seo-rotator-for-images' ) . '</h2><ul>';
		...
		$exclude_query  = "SELECT DISTINCT( post_id ) FROM wp_postmeta
												WHERE ( ( meta_key = '_super_wpseo_sitemap-html-include' AND meta_value = 'never' )
												  OR ( meta_key = '_super_wpseo_meta-robots-noindex' AND meta_value = 1 ) )
												AND post_id NOT IN
													( SELECT pm2.post_id FROM wp_postmeta pm2
															WHERE pm2.meta_key = '_super_wpseo_sitemap-html-include' AND pm2.meta_value = 'always')
	...
	// create post list
	if ( $display_posts ) {
		$output .= '<h2 id="posts">' . __( 'Posts', 'seo-rotator-for-images' ) . '</h2><ul>';
		...
		foreach ( $cats as $cat ) {
			...
			$args = array(
				...
				'meta_query'     => array(
					...
					array(
						'key'     => '_super_wpseo_sitemap-html-include',
						'value'   => 'always',
						'compare' => '='
					)
				)
	...
	foreach ( $post_types as $post_type ) {
		if ( ! in_array( 'noindex-' . $post_type->name, $noindex ) ) {
			$output .= '<h2 id="' . $post_type->name . '">' . __( $post_type->label, 'seo-rotator-for-images' ) . '</h2><ul>';
	...
	if ( $display_archives ) {
		$output .= '<h2 id="archives">' . __( 'Archives', 'seo-rotator-for-images' ) . '</h2><ul>';
