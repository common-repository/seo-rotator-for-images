...
function wpseo_get_value( $val, $postid = 0 ) {
	...
	if ( !empty( $custom['_super_wpseo_' . $val][0] ) )
		return maybe_unserialize( $custom['_super_wpseo_' . $val][0] );
	else
		return false;
}
...
function wpseo_set_value( $meta, $val, $postid ) {
	update_post_meta( $postid, '_super_wpseo_' . $meta, $val );
}
...
function wpseo_replace_vars( $string, $args, $omit = array() ) {
	...
	$replacements = array(
		...
		'%%page%%'         => ( $max_num_pages > 1 && $pagenum > 1 ) ? sprintf( $sep . ' ' . __( 'Page %d of %d', 'seo-rotator-for-images' ), $pagenum, $max_num_pages ) : '',
...
/**
 * (Un-)schedule the super tracking cronjob if the tracking option has changed
 * 
 * Needs to be done here, rather than in the super_Tracking class as class-tracking.php may not be loaded
 *
 * @todo - check if this has any impact on other super plugins which may use the same tracking schedule
 * hook. If so, may be get any other super plugin options, check for the tracking status and
 ...
 * @return	void
 */
function schedule_super_tracking( $disregard, $value ) {
	$current_schedule = wp_next_scheduled( 'super_tracking' );
	$tracking = ( isset( $value['super_tracking'] ) && $value['super_tracking'] ) ? true : false;

	if( $tracking === true && $current_schedule === false ) {
		// The tracking checks daily, but only sends new data every 7 days.
		wp_schedule_event( time(), 'daily', 'super_tracking' );
	}
	else if( $tracking === false && $current_schedule !== false ){
		wp_clear_scheduled_hook( 'super_tracking' );
	}
}
add_action( 'add_option_wpseo', 'schedule_super_tracking', 10, 2 );
add_action( 'update_option_wpseo', 'schedule_super_tracking', 10, 2 );
