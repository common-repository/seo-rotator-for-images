...
/**
 * Main frontend class for Super SEO, responsible for the SEO output as well as removing default WordPress output.
 *
...
class WPSEO_Frontend {
	...
	function title( $title, $sepinput = '-', $seplocation = '' ) {
		...
		if ( $this->is_home_static_page() ) {
			...
			if ( empty( $title ) )
				$title_part = sprintf( __( 'Search for "%s"', 'seo-rotator-for-images' ), esc_html( get_search_query() ) );
		}
			...
			if ( empty( $title ) ) {
				if ( is_month() )
					$title_part = sprintf( __( '%s Archives', 'seo-rotator-for-images' ), single_month_title( ' ', false ) );
				else if ( is_year() )
					$title_part = sprintf( __( '%s Archives', 'seo-rotator-for-images' ), get_query_var( 'year' ) );
				else if ( is_day() )
					$title_part = sprintf( __( '%s Archives', 'seo-rotator-for-images' ), get_the_date() );
				else
					$title_part = __( 'Archives', 'seo-rotator-for-images' );
			}
		}
		else if ( is_404() ) {
			...
			if ( 0 !== get_query_var( 'year' ) || ( 0 !== get_query_var( 'monthnum' ) || 0 !== get_query_var( 'day' ) ) ) {
				...
				if ( empty( $title ) )
					$title_part = __( 'Page not found', 'seo-rotator-for-images' );
			}
		}
		else {
	...
	public function debug_marker( $echo = true ) {
		$marker = "<!-- This site is optimized with the Super SEO plugin v" . WPSEO_VERSION . " - http://superseo.com/wordpress/seo/ -->";
		if ( $echo === false )
			return $marker;
		else
			echo "\n${marker}\n";
	...
	public function metadesc( $echo = true ) {
		...
		if ( $echo !== false ) {
			if ( ! empty( $metadesc ) )
				...
				echo '<!-- ' . __( 'Admin only notice: this page doesn\'t show a meta description because it doesn\'t have one, either write it for this page specifically or go into the SEO -> Titles menu and set up a template.', 'seo-rotator-for-images' ) . ' -->' . "\n";
		}
		else {
			return $metadesc;
		}
	...
	function title_test_helper( $title ) {
		...
		global $wp_version;
		if ( $_SERVER['HTTP_USER_AGENT'] == "WordPress/${wp_version}; " . get_bloginfo( 'url' ) . " - super" )
			return 'This is a super Test Title';
		return $title;
	}
