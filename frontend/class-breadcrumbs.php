...
class WPSEO_Breadcrumbs {
	...
	function breadcrumb( $before = '', $after = '', $display = true ) {
		...
		$links = array(
			array(
				'url'  => get_home_url(),
				'text' => ( isset( $options['breadcrumbs-home'] ) && $options['breadcrumbs-home'] != '' ) ? $options['breadcrumbs-home'] : __( 'Home', 'seo-rotator-for-images' )
			)
		);
		...
		if ( ( $on_front == "page" && is_front_page() ) || ( $on_front == "posts" && is_home() ) ) {
			...
			if ( is_post_type_archive() ) {
				...
				if ( isset( $options['breadcrumbs-archiveprefix'] ) )
					...
					$bc = __( 'Archives for', 'seo-rotator-for-images' );
				if ( is_day() ) {
					global $wp_locale;
					$links[] = array(
				...
				if ( isset( $options['breadcrumbs-searchprefix'] ) && $options['breadcrumbs-searchprefix'] != '' )
					...
					$bc = __( 'You searched for', 'seo-rotator-for-images' );
				...
				if ( 0 !== get_query_var( 'year' ) || ( 0 !== get_query_var( 'monthnum' ) || 0 !== get_query_var( 'day' ) ) ) {
					...
					if ( isset( $options['breadcrumbs-404crumb'] ) && '' != $options['breadcrumbs-404crumb'] )
						...
						$crumb404 = __( 'Error 404: Page not found', 'seo-rotator-for-images' );
...
if ( !function_exists( 'super_breadcrumb' ) ) {
	/**
	 ...
	 * @return string
	 */
	function super_breadcrumb( $before = '', $after = '', $display = true ) {
		global $wpseo_bc;
