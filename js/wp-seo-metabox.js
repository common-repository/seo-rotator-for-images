...
function testFocusKw() {
	...
	var focuskw = jQuery.trim(jQuery('#super_wpseo_focuskw').val());
	...
	var metadesc = jQuery('#super_wpseo_metadesc').val();
	if (metadesc == '')
...
function updateTitle(force) {
	if (jQuery("#super_wpseo_title").val()) {
		var title = jQuery("#super_wpseo_title").val();
	} else {
	...
	jQuery('#super_wpseo_title-length').html(len);
	testFocusKw();
}

function updateDesc(desc) {
	var autogen = false;
	var desc = jQuery.trim(spr_clean(jQuery("#super_wpseo_metadesc").val()));
	var color = '#000';

	if (desc == '') {
		if (wpseo_metadesc_template != '') {
			var excerpt = spr_clean(jQuery("#excerpt").val());
	...
	jQuery('#super_wpseo_metadesc-length').html(len);
...
function boldKeywords(str, url) {
	focuskw = jQuery.trim(jQuery('#super_wpseo_focuskw').val());
...
jQuery(document).ready(function () {
	...
	var desc = jQuery.trim(spr_clean(jQuery("#super_wpseo_metadesc").val()));
	desc = jQuery('<div />').html(desc).text();
	jQuery("#super_wpseo_metadesc").val(desc);
	...
	jQuery('#super_wpseo_focuskw').autocomplete({
		minLength   : 3,
	...
	jQuery('#super_wpseo_title').keyup(function () {
		updateTitle();
	});
	jQuery('#super_wpseo_metadesc').keyup(function () {
		updateDesc();
	});
	...
	jQuery(document).on('change', '#super_wpseo_title', function () {
		updateTitle();
	});
	jQuery(document).on('change', '#super_wpseo_metadesc', function () {
		updateDesc();
	});
	...
	jQuery(document).on('change', '#super_wpseo_focuskw', function () {
		if (jQuery('#super_wpseo_focuskw').val().search(',') != -1) {
